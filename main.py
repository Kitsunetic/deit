...
def get_args_parser():
    ...
    parser.add_argument(
        "--lr-noise-pct", type=float, default=0.67, metavar="PERCENT", help="learning rate noise limit percent (default: 0.67)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--mixup-prob", type=float, default=1.0, help="Probability of performing mixup or cutmix when either/both is enabled"
    )
    parser.add_argument(
        "--mixup-switch-prob",
        type=float,
    ...
    parser.add_argument(
        "--data-set", default="IMNET", choices=["CIFAR", "IMNET", "INAT", "INAT19"], type=str, help="Image Net dataset path"
    )
    parser.add_argument(
        "--inat-category",
        default="name",
    ...
    parser.add_argument(
        "--pin-mem", action="store_true", help="Pin CPU memory in DataLoader for more efficient (sometimes) transfer to GPU."
    )
